...
Dialog.create("Intro");
Dialog.addMessage("* Activate Bio-Formats(Windowless) import option\n     prior to runing this Macro for the first time\n \n* Images for analysis need to be in a separate folder,\n     without any other files of subfolders inside.\n \n* Store results in a different folder");
Dialog.addRadioButtonGroup("Do you run this Macro on:", newArray("PC", "Mac"), 1, 2, "PC");
Dialog.show();
mac=Dialog.getRadioButton();

//  specify folders
if(mac=="Mac") {showMessage("In the next step choose/create Results Folder");	}
...
if(mac=="Mac") {showMessage("In the next step select Source Folder");	}
